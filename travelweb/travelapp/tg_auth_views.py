...
def _build_data_check_string(params: Dict[str, Any]) -> str:
    """
    Сборка data_check_string по документации Telegram.

    ВАЖНО:
    - Берём точные значения из входных параметров.
    - Для 'user' используем СЫРОЕ строковое значение, пришедшее в initData,
      без повторной сериализации (иначе порядок ключей может поменяться и HMAC не сойдётся).
    - Исключаем служебные ключи (hash) и любые наши внутренние (начинаются с '_').
    """
    items = []
    for key in sorted(k for k in params.keys() if k != "hash" and not k.startswith("_")):
        if key == "user" and "_user_raw" in params and isinstance(params["_user_raw"], str):
            val = params["_user_raw"]  # сырое значение user из initData
        else:
            v = params[key]
            # Ничего дополнительно не сериализуем — используем строковое представление.
            val = str(v)
...
def _parse_init_data(init_data_raw: str) -> Dict[str, Any]:
    ...
    Возвращаем словарь, где:
    - 'user' остаётся словарём (для удобства дальнейшей работы),
    - '_user_raw' содержит ТОЧНУЮ строку user ИЗ initData для HMAC.
    """
    ...
    # Сохраняем сырое значение user для HMAC
    if "user" in flat:
        flat["_user_raw"] = flat["user"]
        try:
            flat["user"] = json.loads(flat["user"])
        except Exception:
            # Если парсинг не удался — оставим как есть, но сырое значение всё равно есть
            pass
...
def _extract_telegram_user(params: Dict[str, Any]) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
    ...
    - WebApp initData: params['user'] — dict (а params['_user_raw'] использован только для HMAC)
    ...
    # fallback — плоские поля (вариант Login Widget)
...
def _ensure_local_user(tg: Dict[str, Any]) -> User:
    ...
    # Обновим first/last name, если пришли и отличаются
...
def telegram_login(request: HttpRequest) -> JsonResponse:
    ...
    init_data_raw = (request.POST.get("initData") or "").strip()

    # Нормализуем параметры
    if init_data_raw:
        params = _parse_init_data(init_data_raw)
    else:
        # Fallback: Login Widget (присылает плоские поля)
        keys = ("id", "username", "first_name", "last_name", "auth_date", "hash")
        params = {k: (request.POST.get(k) or "").strip() for k in keys if k in request.POST}
    ...
    # Проверка подписи (использует сырое значение user из initData, если есть)
    ...
    # Вернём avatar_url, если уже есть в профиле (опционально)
